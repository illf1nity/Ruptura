...
module.exports = function createLegacyRouter({ db, timeAgo, fetchChangeOrgData, isValidChangeOrgUrl, isValidTelegramUrl }) {
  ...
  router.post('/api/sentiment/vote', (req, res) => {
    ...
    const fingerprint = typeof req.body.fingerprint === 'string'
      ? req.body.fingerprint.slice(0, 128)
      : null;
  ...
  router.post('/api/buildings/:id/messages', (req, res) => {
    ...
    if (author && (typeof author !== 'string' || author.length > 100)) {
      return res.status(400).json({ error: 'author must be under 100 characters' });
    }
    const building = db.prepare('SELECT * FROM buildings WHERE id = ?').get(id);
    if (!building) {
      return res.status(404).json({ error: 'Building not found' });
  ...
  router.post('/api/worker-collectives', (req, res) => {
    ...
    if (typeof company !== 'string' || company.length > 200) {
      return res.status(400).json({ error: 'company must be under 200 characters' });
    }
    if (typeof industry !== 'string' || industry.length > 200) {
  ...
  router.post('/api/petitions', async (req, res) => {
    ...
    if (typeof area !== 'string' || area.length > 200) {
      return res.status(400).json({ error: 'area must be under 200 characters' });
    }
  ...
  router.post('/api/reports', (req, res) => {
    ...
    if (!['landlord', 'employer'].includes(type)) {
      return res.status(400).json({ error: 'type must be "landlord" or "employer"' });
    }
    if (name.length > 200) {
    ...
    const sanitizedIssues = Array.isArray(issues)
      ? issues.slice(0, 20).map(i => String(i).slice(0, 200))
      : [];
    ...
    ).run(id, type, name, address || null, JSON.stringify(sanitizedIssues));
    res.json({
      id,
      type,
      name,
