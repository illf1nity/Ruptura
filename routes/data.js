...
module.exports = function createDataRouter({ db, fetchCPIData, fetchHUDRentData, fetchHUDStateData, YEARLY_ECONOMIC_DATA, STATE_META }) {
  ...
  router.get('/api/economic-data/cpi', async (req, res, next) => {
    const currentYear = new Date().getFullYear();
    const start = Math.max(1975, Math.min(parseInt(req.query.startYear) || 1975, currentYear));
    const end = Math.max(start, Math.min(parseInt(req.query.endYear) || currentYear, currentYear));
  ...
  router.get('/api/rent-data/hud/:zipCode', async (req, res, next) => {
  ...
  router.get('/api/rent-data/hud/state/:stateCode', async (req, res, next) => {
