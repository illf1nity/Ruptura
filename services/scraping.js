...
function fetchChangeOrgData(url, depth) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.get(url, {
      ...
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        if (!isValidChangeOrgUrl(res.headers.location)) {
          return reject(new Error('Redirect to non-Change.org host blocked'));
        }
        return fetchChangeOrgData(res.headers.location, depth + 1).then(resolve).catch(reject);
      }
...
function fetchTelegramData(url, depth) {
  if (depth === undefined) depth = 0;
  return new Promise((resolve, reject) => {
    if (!url || url.trim() === '') {
    ...
    const req = https.get(url, {
      ...
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        if (!isValidTelegramUrl(res.headers.location)) {
          return reject(new Error('Redirect to non-Telegram host blocked'));
        }
        return fetchTelegramData(res.headers.location, depth + 1).then(resolve).catch(reject);
      }
      if (res.statusCode !== 200) {
        return reject(new Error(`HTTP ${res.statusCode}`));
      }
